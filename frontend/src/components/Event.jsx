                  ...
                  <div className="position-relative text-white" style={{ maxWidth: "1140px", width: "100%", margin: "auto", padding: "0 1.5rem" }}>
                    <div className="row align-items-center">
                      ...
                      <div className="col-md-6">
                        <p className="mb-2 fs-5 fw-semibold text-black">
                          {new Date(event.event_scheduled_date).toLocaleString()}
                        </p>
                        <h2 className="lh-sm fs-2 fw-bold text-black">{event.event_title}</h2>
                        <h4 className="fs-5 fw-bold text-black">₹{event.event_price}</h4>
                        <Link to={`/event-list/${event.event_id}`}>
                        <button
                          className="btn bg-black fs-5 mt-3 p-3 px-5 text-white"
                          style={{ width: "200px", height: "64px", borderRadius: "16px" }}
                        >
                          Book Now
                        </button>
                        </Link>
  ...
  <div className="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 g-4">
    {events.map((event, index) => (
      <div className="col" key={event.event_id}>
        <div className="card shadow-sm" style={{borderRadius: "16px", width: "100%", maxWidth: "304px", height: "543px", margin: "auto"}}>
          ...
          <div className="card-body d-flex flex-column justify-content-between">
            <p className="card-text text-muted fs-6 fw-medium">{new Date(event.event_scheduled_date).toLocaleString()}</p>
            <h5 className="fw-bolder fs-6 overflow-hidden text-wrap lh-sm my-0">{event.event_title}</h5>
            <p className="text-muted fs-6 fw-semibold overflow-hidden text-wrap my-0">₹{event.event_price}</p>
          </div>
        </div>
      </div>
      ))}
